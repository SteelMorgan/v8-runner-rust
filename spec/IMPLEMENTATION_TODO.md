...
- [ ] `ADR-TASK-003`: Ввести единую transport-neutral policy таймаутов и отмены по `ADR-0013` и `ADR-0014`: общий публичный `execution_timeout`, `deadline` для каждой CLI/MCP команды, семантика terminal-state, классы interruption safety, command-boundary cancellation без state machine на каждом step, deferred warning при successful critical phase и наследование budget во вложенных сценариях. Частично выполнено `2026-04-22`: добавлен общий public `execution_timeout` в конфиг и transport-neutral `ExecutionContext`/`McpCallContext` с `deadline` и cancellation token; все public CLI entrypoint-ы инициализируют execution context с общим deadline, MCP queued timeout/cancel на server boundary корректно различает стадии и причины, а CLI теперь маршрутизирует `Ctrl+C`/`SIGTERM` в общий cancellation token. Добавлены `InterruptionSafetyClass` и общий `ProcessExecutionPolicy`; `ProcessExecutor` теперь ждет terminal state при running cancellation/timeout, умеет `Interruptible`/`GracefulThenKill`/`CriticalNonAbortable` semantics и сохраняет deferred interruption metadata для successful critical phase. `load` и `dump` переведены на context-aware process policy: `load` ставит safe point перед `update_db_cfg` и сохраняет deferred warnings в result/execution diagnostics, `dump` проверяет safe point перед publish, чистит staging при interruption до publish и добавляет deferred publication warning; one-shot Designer/EDT syntax path также наследует общий process policy и command budget. Остаются открыты: MCP stdio transport running timeout/cancel всё ещё возвращаются как transport error до финального server result; `build`/`artifacts`/`init`/`extensions` и enterprise test runner ещё не доведены до той же полноты safe-point checks и critical-phase semantics; interactive EDT path всё ещё живет отдельно от общей execution policy.
