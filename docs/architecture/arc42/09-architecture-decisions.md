...
| [ADR-0010: Единый CLI output для человека и AI-агента](../../decisions/0010-razdelit-cli-output-dlya-cheloveka-i-ai-agenta.md) | `accepted`, `2026-04-20` | CLI использует единый high-signal output contract для обеих ролей; `--json-message` выбирает structured JSON output, а `--output` резервируется для user-facing output paths. |
