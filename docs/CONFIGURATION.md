...
`v8-runner tools download` может заполнить этот блок в `v8project.local.yaml`: при
`--extensions sources` он указывает `source.path` на
`build/tools/onec-client-mcp-devkit/exts/client-mcp` и `source.format: EDT`, при
`--extensions artifacts` указывает `artifact.path` на скачанный `client_mcp.cfe`.
Artifact-режим доступен только для `builder=DESIGNER`; для `builder=IBCMD` используйте
source-режим.
...
`v8-runner tools download` заполняет `tools.va.epf_path` в `v8project.local.yaml` путём
